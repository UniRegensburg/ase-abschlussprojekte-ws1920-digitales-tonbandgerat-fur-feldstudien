...
interface LabelDao {
    ...
    @Query("SELECT R.uid, R.recordingName, R.recordingPath, R.date, R.duration,  R.folder, GROUP_CONCAT(L.labelName,', ') AS labels FROM recordingsTable R LEFT JOIN labelAssignmentTable A ON R.uid = A.recordingId LEFT JOIN labelsTable L ON L.uid = A.labelId GROUP BY R.uid ORDER BY R.uid DESC")
